...
-- Insert sample episodes for Friends
INSERT INTO public.episodes (show_id, title, description, season_number, episode_number, duration_minutes, vocabulary_words) 
SELECT 
  s.id,
  'The One Where Monica Gets a Roommate',
  'Rachel leaves her fiancé at the altar and moves in with Monica. We meet the group for the first time.',
  1,
  1,
  22,
  ARRAY['roommate', 'fiancé', 'altar', 'waitress', 'apartment']
FROM public.shows s WHERE s.title = 'Friends';

INSERT INTO public.episodes (show_id, title, description, season_number, episode_number, duration_minutes, vocabulary_words) 
SELECT 
  s.id,
  'The One with the Sonogram at the End',
  'Ross finds out his ex-wife is pregnant. Monica goes on a date with Paul the Wine Guy.',
  1,
  2,
  22,
  ARRAY['sonogram', 'pregnant', 'ex-wife', 'divorce', 'wine']
FROM public.shows s WHERE s.title = 'Friends';